...
StatusOr<GroupOffsets> ReadGroupOffsets(JxlMemoryManager* memory_manager,
  ...
  std::vector<coeff_order_t> permutation;
  JXL_RETURN_IF_ERROR(ReadToc(memory_manager, toc_entries, reader,
                              group_offsets.sizes, permutation));

  group_offsets.offsets.assign(toc_entries, {});

  // Prefix sum starting with 0 and ending with the offset of the last group
  uint64_t offset = 0;
  for (size_t i = 0; i < toc_entries; ++i) {
    if (offset + group_offsets.sizes[i] < offset) {
      return JXL_FAILURE("group offset overflow");
    }
    group_offsets.offsets[i] = offset;
    offset += group_offsets.sizes[i];
  }
  group_offsets.total_size = offset;
  ...
  if (!permutation.empty()) {
    ...
    for (coeff_order_t index : permutation) {
      permuted_offsets.push_back(group_offsets.offsets[index]);
      permuted_sizes.push_back(group_offsets.sizes[index]);
    }
    std::swap(group_offsets.offsets, permuted_offsets);
    std::swap(group_offsets.sizes, permuted_sizes);
  }
