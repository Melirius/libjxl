...
Status ReadPermutation(size_t skip, size_t size, coeff_order_t* order,
  ...
  uint32_t end = reader.ReadHybridUint(CoeffOrderContext(size)) + skip;
  if (end > size) {
    return JXL_FAILURE("Invalid permutation size");
  }
  uint32_t last = 0;
  for (size_t i = skip; i < end; ++i) {
    lehmer[i] = reader.ReadHybridUint(CoeffOrderContext(last));
    last = lehmer[i];
    if (lehmer[i] >= size - i) {
...
Status DecodePermutation(JxlMemoryManager* memory_manager, size_t skip,
  ...
  JXL_ASSIGN_OR_RETURN(
      ANSCode code, DecodeHistograms(memory_manager, br, kPermutationContexts));
  ANSSymbolReader reader;
  JXL_RETURN_IF_ERROR(reader.Init(code, br));
  JXL_RETURN_IF_ERROR(ReadPermutation(skip, size, order, reader));
...
Status DecodeCoeffOrder(AcStrategy acs, coeff_order_t* order,
                        ANSSymbolReader& reader,
  ...
  JXL_RETURN_IF_ERROR(ReadPermutation(llf, size, order, reader));
  if (order == nullptr) return true;
...
Status DecodeCoeffOrders(JxlMemoryManager* memory_manager, uint16_t used_orders,
  ...
  if (used_orders != 0) {
    JXL_ASSIGN_OR_RETURN(
        code, DecodeHistograms(memory_manager, br, kPermutationContexts));
    JXL_RETURN_IF_ERROR(reader.Init(code, br));
  }
  uint32_t acs_mask = 0;
  ...
  for (uint8_t o = 0; o < AcStrategy::kNumValidStrategies; ++o) {
    ...
    if ((used_orders & (1 << ord)) == 0) {
      ...
      for (size_t c = 0; c < 3; c++) {
        ...
        JXL_RETURN_IF_ERROR(DecodeCoeffOrder(acs, dest, reader, natural_order));
      }
    }
  }
