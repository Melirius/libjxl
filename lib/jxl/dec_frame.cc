...
Status FrameDecoder::InitFrame(BitReader& br, ImageBundle* decoded,
                               bool is_preview) {
  decoded_ = decoded;
...
Status FrameDecoder::ProcessACGlobal(BitReader& br) {
  ...
  if (frame_header_.encoding == FrameEncoding::kVarDCT) {
    ...
    dec_state_->shared_storage.num_histograms = 1 + br.ReadBits(num_histo_bits);
    ...
    for (size_t i = 0; i < frame_header_.passes.num_passes; i++) {
      ...
      JXL_ASSIGN_OR_RETURN(dec_state_->code[i],
...
Status FrameDecoder::ProcessACGroup(size_t ac_group_id,
                                    BitReader* JXL_RESTRICT* readers,
  ...
  if (frame_header_.encoding == FrameEncoding::kVarDCT) {
    ...
    JXL_RETURN_IF_ERROR(DecodeGroup(
        frame_header_, readers, num_passes, ac_group_id, dec_state_,
  ...
  for (size_t i = pass0; i < pass1; ++i) {
    ...
    if (i < pass0 + num_passes) {
      JXL_DEBUG_V(2, "Bit reader position: %" PRIuS " / %" PRIuS,
                  readers[i - pass0]->TotalBitsConsumed(),
                  readers[i - pass0]->TotalBytes() * kBitsPerByte);
      JXL_RETURN_IF_ERROR(modular_frame_decoder_.DecodeGroup(
          frame_header_, mrect, readers[i - pass0], minShift, maxShift,
